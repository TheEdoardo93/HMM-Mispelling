...
class calcoloVettoreMatrici():
    ...
    def calcolo_matrice_transizioni(self):
        tweets = open("./PerturbazioneTweet/training_puliti.txt", "r")
        ...
        for line in tweets.readlines():
            for i in range(1, len(line)):
                ...
                if ((character.isalpha() == True) and (previous_character.isalpha() == True)):
                    matrice_T[(ord(previous_character) - ord('a'), ord(character) - ord('a'))] += 1.0
                    ...
                    elif ((previous_character.isspace() == True) and (character.isspace() == True)):
                        matrice_T[26, 26] += 1.0
    ...
    def creazione_modello(self, matrice_T, matrice_O, vettore_Pi):
    ...
    def delete__by_values(self, lst, values):
        values_as_set = set(values)
        return [ x for x in lst if x not in values_as_set ]
        
        
    def inferenza(self, model, sequence):
        unlist=['\x82', '\xac','\x87', '\xbd', '\xbe', '\xb6', '\xa4',
                ...
                ',', '.', '-', '"', '(', ')', '1', '2', '3', '4', '5', 
                '6', '7', '8', '9', '0']
        shortSequence = self.delete__by_values(sequence, unlist)
        
        logp, path = model.viterbi(shortSequence[0:len(shortSequence)])
        #print("VITERBI")
        ...
        for i in range(0, len(sequence)):
            if sequence[i].isalpha() == False and sequence[i].isspace() == False: 
                x.insert(i, sequence[i])
    ...
    #vettore_Pi = calcolo_vettore_pi(self)
    #matrice_T = calcolo_matrice_transizioni(self,)
    #matrice_O = calcolo_matrice_osservazioni(self)
    #model = creazione_modello(self, matrice_T, matrice_O, vettore_Pi)
    #inferenza(self, model, list("zs yoi see thwre ix a sea begween you abd mr"))
    #inferenza(self, model, list("qe zll ljvr ih tue ywllow xubmarine"))
    #errore = test(self, model)
